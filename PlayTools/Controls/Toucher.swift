...
class SimpleToucher {
    ...
    static var touchQueue = DispatchQueue.init(label: "playcover.simpletoucher", qos: .userInteractive)

    
    static func touch(point: CGPoint, id: Int, phase: UITouch.Phase) {
        touchQueue.async {
            ...
            PTFakeMetaTouch.fakeTouchId(id, at: point, with: phase, in: keyWindow, on: keyView)
        }
    }
}
